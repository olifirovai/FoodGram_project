...
from recipe.models import Recipe, FavoriteRecipe, ShoppingList, RecipeTypeMapping
...
def is_in_type(type, recipe):
    type_exists = RecipeTypeMapping.objects.filter(type=type, recipe=recipe).exists()
    return type_exists
